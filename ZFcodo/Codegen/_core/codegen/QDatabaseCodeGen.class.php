<?php
	...
	class QDatabaseCodeGen extends QCodeGen {
		...
		public function __construct($objSettingsXml) {
			...
			$this->strBaseClassPrefix = QCodeGen::LookupSetting($objSettingsXml, 'baseClassName', 'prefix');
			$this->strFileNamePrefix = QCodeGen::LookupSetting($objSettingsXml, 'fileName', 'prefix');
...
//						$objReferencedTable = $this->objTableArray[strtolower($objReference->Table)];
		...
		public function GenerateTable(QTable $objTable) {
			...
			return $this->GenerateFiles('zend_orm', $mixArgumentArray);
		}
...
//				$this->objTableArray[strtolower($strTableWithReference)]->ManyToManyReferenceArray = $objArray;
		...
		protected function AnalyzeTypeTable(QTypeTable $objTypeTable) {
			...
			$objTypeTable->BaseClassName = $this->BaseClassNameFromTableName($strTableName);
            $objTypeTable->FileName = QConvertNotation::CamelCaseFromUnderscore($this->StripPrefixFromTable($strTableName));
